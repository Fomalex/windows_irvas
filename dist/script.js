...
window.addEventListener('DOMContentLoaded', function () {
  var modalState = {};
  var deadline = '2021-12-18T00:00:00';
...
var forms = function forms(state) {
  var form = document.querySelectorAll('form'),
      inputs = document.querySelectorAll('input'); //чтобы в инпутах с номером вводились только цифры:
...
var modals = function modals() {
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    var isFilled = true; //Добавление проверок на валидацию:

    function validateInputs(selector, previousSelector, event) {
      if (modalSelector === selector) {
        var previousModal = document.querySelector(previousSelector);
        var formInputs = previousModal.querySelectorAll('input');
        isFilled = false;
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  function hideTabContent() {
    ...
    tab.forEach(function (item) {
      item.classList.remove(activeClass); //Частные случаи дополнения к цвету ссылок:
  ...
  function showTabContent() {
    ...
    tab[i].classList.add(activeClass); //Частные случаи дополнения к цвету ссылок:
